...
if (isset($_POST['login'])) {
    ...
    if (empty($username) || empty($password)) {
        ...
        $check_user = $conn->prepare("SELECT * FROM auth WHERE username =? AND role = 'official'");
        ...
        if (!$results->num_rows > 0) {
            ...
            if (password_verify($password, $user_password)) {
                ...
                $update_at = $conn->prepare("UPDATE auth SET update_at = CURRENT_TIMESTAMP WHERE username = ? AND role = 'official'");
...
<section class="container-fluid pb-5">
    <div class="container my-3 shadow rounded px-md-5 px-2 py-4 bg-white mt-5" style="max-width: 30rem;">
        <div class="text-center mb-5">
            ...
            <p class="fw-semibold mb-1">Login Officially to control voter activities</p>
        </div>
        <form method="post">
