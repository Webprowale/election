...
if (isset($_POST['login'])) {
    ...
    if (empty($username) || empty($password)) {
        ...
        $check_user = $conn->prepare("SELECT * FROM auth WHERE username =? AND role = 'admin'");
        ...
        if (!$results->num_rows > 0) {
            ...
            if (password_verify($password, $user_password)) {
                ...
                $update_at = $conn->prepare("UPDATE auth SET update_at = CURRENT_TIMESTAMP WHERE username = ? AND role = 'admin'");
